...
def testCreateUpdateGetDeleteTask():
    files: Files = client.uploads.upload_file(attributes=UploadFileAttributesArg(name=get_uuid(), parent=UploadFileAttributesArgParentField(id='0')), file=generate_byte_stream(10))
    file: Files = files.entries[0]
    task: Task = client.tasks.create_task(item=CreateTaskItemArg(type=CreateTaskItemArgTypeField.FILE.value, id=file.id), action=CreateTaskActionArg.REVIEW.value, message='test message', due_at='2035-01-01T00:00:00Z', completion_rule=CreateTaskCompletionRuleArg.ALL_ASSIGNEES.value)
    ...
    updated_task: Task = client.tasks.update_task_by_id(task_id=task.id, message='updated message')
