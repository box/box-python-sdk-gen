...
def testTrashedFolders():
    ...
    from_trash: TrashFolder = client.trashed_folders.get_folder_trash(folder_id=folder.id)
    assert from_trash.id == folder.id
    assert from_trash.name == folder.name
    with pytest.raises(Exception):
        client.folders.get_folder_by_id(folder_id=folder.id)
    restored_folder: TrashFolderRestored = client.trashed_folders.restore_folder_from_trash(folder_id=folder.id)
