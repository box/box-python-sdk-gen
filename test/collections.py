...
def testCollections():
    ...
    favourite_collection: Collections = collections.entries[0]
    collection_items: Items = client.collections.get_collection_items(collection_id=favourite_collection.id)
    folder: FolderFull = client.folders.create_folder(name=get_uuid(), parent=CreateFolderParentArg(id='0'))
    client.folders.update_folder_by_id(folder_id=folder.id, collections=[UpdateFolderByIdCollectionsArg(id=favourite_collection.id)])
    collection_items_after_update: Items = client.collections.get_collection_items(collection_id=favourite_collection.id)
    assert len(collection_items_after_update.entries) == len(collection_items.entries) + 1
    client.folders.update_folder_by_id(folder_id=folder.id, collections=[])
    collection_items_after_remove: Items = client.collections.get_collection_items(collection_id=favourite_collection.id)
