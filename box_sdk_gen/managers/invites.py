...
class InvitesManager:
    ...
    def get_invite_by_id(self, invite_id: str, fields: Optional[str] = None) -> Invite:
        ...
        """
        query_params_map: Dict[str, str] = prepare_params({'fields': to_string(fields)})
        response: FetchResponse = fetch(''.join(['https://api.box.com/2.0/invites/', invite_id]), FetchOptions(method='GET', params=query_params_map, response_format='json', auth=self.auth, network_session=self.network_session))
        return Invite.from_dict(json.loads(response.text))