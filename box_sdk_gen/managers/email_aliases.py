...
class EmailAliasesManager:
    ...
    def delete_user_email_alias_by_id(self, user_id: str, email_alias_id: str) -> None:
        ...
        """
        response: FetchResponse = fetch(''.join(['https://api.box.com/2.0/users/', user_id, '/email_aliases/', email_alias_id]), FetchOptions(method='DELETE', response_format=None, auth=self.auth, network_session=self.network_session))
        return None